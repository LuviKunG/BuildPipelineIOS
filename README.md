...
This is a build tools for Unity Editor able to manage build iOS separate by folder and version control (via naming) and able to modify info.plist via unity internal script include URLSchemes.
...
- **iOS** will perform a build. If you're not select the directory before, it will popup the directory selection window and start perform a build.
- **Settings/iOS** will open the build pipeline settings.
...
{
  "dependencies": {
    "com.luvikung.buildpipelineios": "https://github.com/LuviKunG/BuildPipelineIOS.git#1.0.1"
  }
}
```
...
**https://github.com/LuviKunG/BuildPipelineIOS.git#1.0.1**
